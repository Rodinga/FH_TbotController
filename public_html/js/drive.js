...
function initWebsocket(wsURI) {
  ...
  socket.onopen = function (event) {
    DBLog("Websocket Connected");
    setBroadcast();
  };
  socket.onclose = function (event) {
    DBLog("Websocket Lost, Retrying.")
  ...
  socket.onerror = function (event) {
    DBLog("Websocket has encountered a error: " + event);
    clearInterval(bIntID);
  }
      ...
      String(event.data).split(',').forEach(function (element) {
        if (element.startsWith('/')) {
          switch (element.charAt(1)) {
            //Battery
            case "B":
              //document.getElementById("voltage").innerHTML =  window.feedback.volt + " V";
              //Refer to bSelection function
              bSelection(element.substr(2), "B");
              break;
            //Voltage
            case "V":
              //document.getElementById("voltage").innerHTML =  window.feedback.volt + " V";
              //Refer to bSelection function
              bSelection(element.substr(2), "V");
              break;
            //Temperature
            case "T":
              //document.getElementById("temperature").innerHTML = element.substr(2) + " �C";
              tSelection(element.substr(2) + " °C");
              break;
            //Distance
            case "D":
